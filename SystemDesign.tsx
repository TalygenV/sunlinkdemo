...
import { getAllPanels, getPanelId, calculateTotalStats, updateActivePanels } from '../utils/panelHelpers';
...
const SystemDesign: React.FC<SystemDesignProps> = ({ userData }) => {
  ...
  const [completedStages, setCompletedStages] = useState<ProgressStage[]>(['Panels']);
  ...
  useEffect(() => {
    const checkProfileStatus = async () => {
      ...
      if (!userData.name || !userData.address) {
        console.log("SystemDesign: User profile incomplete - missing name or address");
  ...
  const [selectedBatteryDetails, setSelectedBatteryDetails] = useState<any>(null);
  ...
  const [manualPanelObstructedIds, setManualPanelObstructedIds] = useState<Set<string>>(new Set());
  // Add alignment saved state
  const [alignmentSaved, setAlignmentSaved] = useState<boolean>(false);
  // Add autoPanelOffset state
  const [autoPanelOffset, setAutoPanelOffset] = useState<{lat: number, lng: number}>({lat: 0, lng: 0});
  
  // Progress persistence states
  const [lastActiveTimestamp, setLastActiveTimestamp] = useState<number>(Date.now());
  const [designSessionState, setDesignSessionState] = useState<'in-progress' | 'completed' | 'abandoned'>('in-progress');
  ...
  const updatePanelData = useCallback(async () => {
    ...
    try {
      ...
      const isAutoSupported = userData.isAutoPanelsSupported !== undefined ?
  ...
  const updateChangedPanelData = useCallback(async () => {
    ...
    try {
      ...
      const currentState: Record<string, any> = {
        ...
        alignmentSaved,
        autoPanelOffset,
        isAutoPanelsSupported: userData.isAutoPanelsSupported !== undefined ?
          userData.isAutoPanelsSupported :
          (solarData?.isAutoPanelsSupported || false),
        selectedBatteryDetails,
        batteryCount,
  ...
  const handleIncreasePanels = () => {
    ...
    setPanels(updateActivePanels(panels, stats.totalPanels + 1, obstructedPanels));
  ...
  const handleDecreasePanels = () => {
    const stats = calculateTotalStats(panels, obstructedPanels);
    setPanels(updateActivePanels(panels, Math.max(0, stats.totalPanels - 1), obstructedPanels));
    // Update database after state change
    ...
    if (solarData) {
      try {
        ...
        if (!roofData && !manualPanelsOn) {
          console.log('No roof data available - switching to manual panels mode');
          setManualPanelsOn(true);
        }
        
        // Use existing panels when reinitializing
        setPanels(prevPanels => getAllPanels(solarData, prevPanels, roofData));
      } catch (err) {
  ...
  useEffect(() => {
    // Skip initial render
    if (manualPanelRegions.length > 0 || totalManualPanels > 0 || manualPanelEnergy > 0 || manualPanelsOn) {
  ...
  }, [manualPanelsOn, obstructedPanels, manualPanelRegions, selectedRegionId, totalManualPanels, manualPanelEnergy, currentRotation]);
  
  // Add useEffect for battery-related state changes
  useEffect(() => {
    // Skip initial render or when battery features are not in use
    if (batteryCount === 0 && !selectedBatteryDetails && !isBatterySkipped) return;
  ...
  const calculateMapCenter = (bounds: { north: number; east: number; south: number; west: number }) => {
    return {
  ...
  const ensureProgressConsistency = () => {
    ...
    const requiredStages = stages.slice(0, currentIndex + 1).filter(stage => stage !== 'Completion') as ProgressStage[];
    const updatedCompletedStages = [...new Set([...completedStages, ...requiredStages])];
    
    if (JSON.stringify(updatedCompletedStages) !== JSON.stringify(completedStages)) {
  ...
  const saveProgressState = useCallback(async (
    ...
    try {
      ...
      await update(progressRef, {
        ...
        purchaseCompleted: currentStageToSave === 'Overview' && selectedPaymentOption === 'cash'
      });
  ...
  const resetProgress = async () => {
    if (!window.confirm('This will reset your design progress. Are you sure?')) {
      return;
    }
      ...
      try {
        ...
        if (hasRequiredSystemDesignData(snapshot)) {
          ...
          if (snapshot.val().progress) {
            ...
            if (progressData.completedStages && Array.isArray(progressData.completedStages)) {
          ...
          } else if (snapshot.val().solarData && snapshot.val().solarData.center &&
                    snapshot.val().solarData.center.latitude && snapshot.val().solarData.center.longitude) {
          ...
          setSolarData(existingSolarData ? {
            ...existingSolarData,
            isAutoPanelsSupported: isAutoSupported
          } : null);
          ...
          if (snapshot.val().manualPanelObstructedIds !== undefined) {
            ...
            setManualPanelObstructedIds(new Set(snapshot.val().manualPanelObstructedIds));
          }
          ...
          if (!snapshot.val().imageUrl) {
            console.log("Existing data found, but imageUrl is missing. Fetching from Nearmap.");
            const address = userData.address;
            if (address) {
              try {
                const functions = getFunctions();
                const checkAvailabilityFunc = httpsCallable(functions, 'checkSurveyAvailability');
                const addressWithoutUSA = address.replace(/, USA$/, '');
                const response = await checkAvailabilityFunc({ address: addressWithoutUSA });
                ...
                if (response.data.success && response.data.data?.imageData) {
                  ...
                  const imageRef = storageRef(storage, `nearmap-images/${formattedAddress}`);
          ...
          if (solarDataFromUser && solarDataFromUser.coordinates &&
              solarDataFromUser.coordinates.latitude && solarDataFromUser.coordinates.longitude) {
          ...
          } else if (solarDataFromUser && solarDataFromUser.center && 
                    solarDataFromUser.coordinates.latitude && solarDataFromUser.coordinates.longitude) {
          ...
          if (solarDataFromUser && solarDataFromUser.solarPotential && userData.isAutoPanelsSupported &&
            ...
            try {
              ...
              if (solarDataFromUser.solarPotential.solarPanelConfigs) {
                ...
                const bestAnalysis = validAnalyses
                  ...
                  .reduce((best: any, current: any) => {
                    if (!solarDataFromUser.targetMonthlyBill) {
                      ...
                      const currentSavings = current.cashPurchaseSavings?.savings?.savingsYear20?.units || '0';
                      const bestSavings = best.cashPurchaseSavings?.savings?.savingsYear20?.units || '0';
                      return parseInt(currentSavings) > parseInt(bestSavings) ? current : best;
                    }
                    
                    // Calculate target monthly bill using the formula: (monthlyBill / 2) + 25
                    const targetBill = (solarDataFromUser.targetMonthlyBill / 2) + 25;
                    
                    // Get the absolute difference between each analysis's monthly bill and target
                    const currentDiff = Math.abs(parseInt(current.monthlyBill?.units || '0') - targetBill);
                    const bestDiff = Math.abs(parseInt(best.monthlyBill?.units || '0') - targetBill);
                ...
                if (bestAnalysis) {
                  const selectedConfig = solarDataFromUser.solarPotential.solarPanelConfigs[bestAnalysis.panelConfigIndex];
              ...
              if (panelCount > 0) {
                initialPanels = updateActivePanels(allPanels, panelCount, new Set());
              } else {
          ...
          const userAnnualUsage = userData.annualUsage ||
            (solarDataFromUser && solarDataFromUser.solarPotential && 
             solarDataFromUser.solarPotential.solarPanelConfigs && 
             solarDataFromUser.solarPotential.solarPanelConfigs[0] ? 
             solarDataFromUser.solarPotential.solarPanelConfigs[0].yearlyEnergyDcKwh : 12000);
          ...
          const hasSolarPanelData = solarDataFromUser &&
  ...
  const handleContinue = async () => {
    ...
    const stages = ['Panels', 'Inverter', 'Batteries', 'Design', 'Overview'] as const;
    ...
    if (currentStage === 'Overview' && selectedPaymentOption  && window.showStripeModal) {
    ...
    if (currentIndex < stages.length - 1) {
      ...
      const updatedCompletedStages = [...new Set([
        ...completedStages,
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <DesignStep
                ...
                // Add autoPanelOffset props
                autoPanelOffset={autoPanelOffset && { 
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <OverviewStep
                ...
                batteryCount={batteryCount}
                selectedBatteryDetails={selectedBatteryDetails}
                solarPanelType={panels[0] ? `${panels[0].brand} ${panels[0].model}` : 'N/A'} // Get panel type
  ...
  return (
    <div className="design-route relative min-h-screen overflow-x-hidden overflow-y-auto lg:overflow-hidden">
      ...
      <ProgressBar
        ...
        className={`absolute ${isMobile ? 'top-[2%]' : 'top-[5%]'} left-0 w-full`}
      />
      
      {/* Step Content */}
      <div className={`relative z-5 ${isMobile ? `pt-[18dvh] max-w-[95dvw] ${currentStage === 'Design' ? 'overflow-y-auto' : ''}` : 'pt-[21dvh] max-w-[88dvw]'}  mx-auto`}>
        {renderStepContent()}
      </div>
            ...
            >
              ...
              <div className="relative z-10 flex items-center justify-end w-full">
                <div className="px-8 py-6 flex items-center gap-3 ">
                  <span className="text-white text-xl">{getContinueButtonText()}</span>
...
const BatteriesStep: React.FC<StepComponentProps> = ({ onContinue, updateCurrentBatteryName, selectedBatteryDetails }) => {
  return (
    <>
      <BatteryShowcase
        updateCurrentBatteryName={updateCurrentBatteryName}
