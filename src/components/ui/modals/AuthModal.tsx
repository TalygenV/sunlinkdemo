import { createUserWithEmailAndPassword, getAuth, RecaptchaVerifier, signInWithEmailAndPassword, signInWithPhoneNumber, updateProfile } from 'firebase/auth';
import { ref, set } from 'firebase/database';
import { doc, setDoc } from "firebase/firestore";
import { AnimatePresence, motion } from 'framer-motion';
import { ArrowRight, CheckCircle, Lock, Mail, Phone, Search, Shield, User, X } from 'lucide-react';
...
interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (userData?: { name: string; address: string; phoneNumber?: string; uid?: string; solarData?: any; monthlyBill?: number; annualUsage?: number; isAutoPanelsSupported?: boolean }) => void;
...
export default function AuthModal({ isOpen, onClose, onSuccess, initialAddress, solarData, monthlyBill, annualUsage, isAutoPanelsSupported }: AuthModalProps) {
      ...
      if (err.code) {
        switch (err.code) {
          ...
          case 'auth/user-not-found':
            userFriendlyError = "No account found with this phone number. Please sign up first.";
            break;
          case 'auth/wrong-password':
            userFriendlyError = "Incorrect verification code. Please try again.";
            break;
          case 'auth/quota-exceeded':
            userFriendlyError = "Too many verification attempts. Please try again later.";
            break;
          default:
      ...
      if (err.code) {
        switch (err.code) {
          ...
          case 'auth/user-disabled':
            ...
            break;
          case 'auth/user-not-found':
            userFriendlyError = "No account found with this verification code. Please sign up first.";
            break;
          case 'auth/wrong-password':
            userFriendlyError = "Incorrect verification code. Please try again.";
            break;
      ...
      if (err.code) {
        switch (err.code) {
          ...
          case 'auth/code-expired':
            userFriendlyError = "Verification code has expired. Please request a new one.";
            break;
          case 'auth/quota-exceeded':
            userFriendlyError = "Too many verification attempts. Please try again later.";
            break;
      ...
      if (err.code) {
        switch (err.code) {
          ...
          case 'auth/user-disabled':
            ...
            break;
          case 'auth/user-not-found':
            userFriendlyError = "No account found with this installation address. Please sign up first.";
            break;
          case 'auth/quota-exceeded':
            userFriendlyError = "Too many verification attempts. Please try again later.";
            break;
    ...
    switch (step) {
      case 'phone':
        ...
        <Phone className="w-12 h-12 text-white relative z-10" />
            ...
            <h2 className="text-2xl font-light text-white mb-2">Sign Up to Continue</h2>
      ...
      case 'verify':
        return (
          <>
            ...
            <h2 className="text-2xl font-light text-white mb-2">Verify Your Number</h2>
            <p className="text-gray-400 text-sm">Enter the verification code sent to your phone</p>
      ...
      case 'complete':
        return (
          <>
            ...
            <h2 className="text-2xl font-light text-white mb-2">Finish Setting Up Your Account</h2>
            ...
            <form onSubmit={handleCompleteProfile} className="mt-8 space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Note: This will be your installation address and can only be changed by contacting support
                </p>
              </div>
              <motion.button
                type="submit"
      ...
      case 'welcome':
        return (
          <>
            ...
            <h2 className="text-3xl font-light text-white mb-4">Welcome, {name}!</h2>
            <p className="text-gray-400 text-lg mb-8">Your account has been successfully created.</p>
            <motion.button
              onClick={() => {
                // Remove the auth in progress flag when profile setup is complete
                localStorage.removeItem('authInProgress');
                
                console.log("DEBUG: solarData in AuthModal before passing to onSuccess:", solarData);
                const userData = {
                  name,
                  address,
