...
import { Search, Filter, ArrowDown, X, Calendar, Clock, User, MapPin, Wrench, Zap, Users } from 'lucide-react';
...
export const InstallerProjectsPage: React.FC<InstallerProjectsPageProps> = ({ isAdmin = false }) => {
  ...
  const [sortBy, setSortBy] = useState<'date' | 'name' | 'status' | 'size'>('date');
    ...
    if (isAdmin) {
      const fetchInstallers = async () => {
        try {
          ...
          const snapshot = await get(installersRef);
          
          if (snapshot.exists()) {
            const installersData = snapshot.val();
            const installersArray: Installer[] = Object.entries(installersData).map(([id, data]: [string, any]) => ({
              id,
              name: data.name || 'Unknown Installer',
      ...
      try {
        ...
        if (isAdmin) {
          ...
          if (installersSnapshot.exists()) {
            ...
            for (const [installerId, installerData] of Object.entries(installersData)) {
              const installerInfo = installerData as any;
              
              if (installerInfo.assignedProjects) {
                const installerProjects = Object.entries(installerInfo.assignedProjects).map(([projectId, projectData]: [string, any]) => ({
                  id: projectId,
                  ...
                  assignedDate: projectData.assignedDate || new Date().toISOString(),
                  ...
                  installerCompany: installerInfo.companyName || 'Unknown Company'
                }));
          ...
          const projectsRef = ref(db, `installers/${user.uid}/assignedProjects`);
          ...
          if (snapshot.exists()) {
            ...
            const projectsArray: Project[] = Object.entries(projectsData).map(([id, data]: [string, any]) => ({
              id,
  ...
  useEffect(() => {
    ...
    if (isAdmin && selectedInstaller) {
      filtered = filtered.filter(project => project.installerId === selectedInstaller);
    }
  ...
  }, [projects, searchQuery, statusFilter, sortBy, sortDirection, selectedInstaller, isAdmin]);
  ...
  const isFilterActive = searchQuery || statusFilter || selectedInstaller || sortBy !== 'date' || sortDirection !== 'desc';
    ...
    return (
      <InstallerPortalLayout isAdmin={isAdmin}>
        <div className="p-6 bg-black/20 backdrop-blur-sm rounded-xl border border-red-500/20">
          <h3 className="text-xl font-medium text-white mb-2">Projects Error</h3>
          ...
          <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {isAdmin && (
              <div className="relative mr-2">
                ...
                >
                  <Users size={16} />
                  <span>{selectedInstaller ?
                    `Installer: ${installers.find(i => i.id === selectedInstaller)?.companyName || 'Unknown'}` :
                    'All Installers'}
                  </span>
                </button>
                  ...
                  <div className="absolute top-full left-0 mt-1 w-64 z-10 bg-orange-900 border border-white/10 rounded-lg shadow-xl overflow-hidden">
                    <div className="p-2">
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-md ${!selectedInstaller ? 'bg-orange-500/20 text-yellow-400' : 'text-white/70 hover:bg-white/5'}`}
                      >
                        All Installers
                      </button>
                      ...
                      {installers.map(installer => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-md ${selectedInstaller === installer.id ? 'bg-orange-500/20 text-yellow-400' : 'text-white/70 hover:bg-white/5'}`}
                        >
            ...
            <div className="relative">
              ...
              >
                <Filter size={16} />
                <span className='mr-2'>{statusFilter ? `Status: ${statusFilter}` : 'Filter by status'}</span>
              </button>
                ...
                <div className="absolute top-full left-0 mt-1 w-48 z-10 bg-orange-900 border border-white/10 rounded-lg shadow-xl overflow-hidden">
                  <div className="p-2">
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-md ${!statusFilter ? 'bg-orange-500/20 text-yellow-400' : 'text-white/70 hover:bg-white/5'}`}
                    >
                      All statuses
                    </button>
                    ...
                    {statusOptions.map(status => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-md ${statusFilter === status ? 'bg-orange-500/20 text-yellow-400' : 'text-white/70 hover:bg-white/5'}`}
                      >
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 sm:mt-0">
              ...
              <button
                ...
                className={`px-2 md:px-3 py-1 md:py-1.5 rounded-md text-xs md:text-sm ${sortBy === 'date' ? 'bg-orange-500/20 text-yellow-400' : 'text-white/70 hover:bg-white/5'}`}
              >
                Date
              </button>
              ...
              <button
                ...
                className={`px-2 md:px-3 py-1 md:py-1.5 rounded-md text-xs md:text-sm ${sortBy === 'name' ? 'bg-orange-500/20 text-yellow-400' : 'text-white/70 hover:bg-white/5'}`}
              >
                Customer
              </button>
              ...
              <button
                ...
                className={`px-2 md:px-3 py-1 md:py-1.5 rounded-md text-xs md:text-sm ${sortBy === 'status' ? 'bg-orange-500/20 text-yellow-400' : 'text-white/70 hover:bg-white/5'}`}
              >
                Status
              </button>
              ...
              <button
                ...
                className={`px-2 md:px-3 py-1 md:py-1.5 rounded-md text-xs md:text-sm ${sortBy === 'size' ? 'bg-orange-500/20 text-yellow-400' : 'text-white/70 hover:bg-white/5'}`}
              >
                Size
              </button>
                ...
                <table className="min-w-full">
                  <thead className="bg-black/90 sticky top-0 ">
                    <tr className="border-b border-white/10">
                      <th className="text-left py-3 px-4 text-white/70 font-medium text-xs md:text-sm bg-black/30">Customer</th>
                      <th className="text-left py-3 px-4 text-white/70 font-medium text-xs md:text-sm hidden md:table-cell bg-black/30">Address</th>
                      <th className="text-left py-3 px-4 text-white/70 font-medium text-xs md:text-sm hidden sm:table-cell bg-black/30">System Size</th>
                      <th className="text-left py-3 px-4 text-white/70 font-medium text-xs md:text-sm hidden sm:table-cell bg-black/30">Assigned Date</th>
                      {isAdmin && (
                        <th className="text-left py-3 px-4 text-white/70 font-medium text-xs md:text-sm hidden md:table-cell bg-black/30">Installer</th>
                      )}
                      <th className="text-left py-3 px-4 text-white/70 font-medium text-xs md:text-sm bg-black/30">Status</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      >
                        <td className="py-3 md:py-4 px-4">
                          <div className="flex items-center gap-2 md:gap-3">
                            ...
                            <div>
                              <p className="text-white font-medium text-sm md:text-base">{project.customerName}</p>
                              <p className="text-white/50 text-xs truncate max-w-[120px] sm:max-w-none">{project.email}</p>
                              {/* Mobile-only address */}
                              <p className="text-white/50 text-xs md:hidden mt-1 truncate max-w-[180px]">{project.address}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-3 md:py-4 px-4 hidden md:table-cell">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-white/90 text-sm truncate max-w-[200px] lg:max-w-none">{project.address}</span>
                          </div>
                        </td>
                        <td className="py-3 md:py-4 px-4 hidden sm:table-cell">
                          <div className="flex items-center gap-2">
                            <Zap size={16} className="text-white/50 flex-shrink-0" />
                            <div>
                              <p className="text-white text-sm">{project.totalManualPanels * 400 / 1000} kW</p>
                              <p className="text-white/50 text-xs">{project.totalManualPanels} panels</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-3 md:py-4 px-4 hidden sm:table-cell">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-white/90 text-sm">{formatDate(project.assignedDate)}</span>
                          </div>
                        </td>
                        {isAdmin && (
                          <td className="py-3 md:py-4 px-4 hidden md:table-cell">
                            {project.installerCompany ? (
                              <div className="flex items-center gap-2">
                                <Users size={16} className="text-white/50 flex-shrink-0" />
                                <span className="text-white/90 text-sm">{project.installerCompany}</span>
                              </div>
                            ) : (
